...
The system operates under at-least-once delivery semantics — all consumers must be idempotent. Every Kafka event handler records the `eventId` in an `inbox_events` table before processing. On redelivery the duplicate is detected and silently skipped, making every consumer safe to retry without risk of double-charging, double-approving, or double-cancelling.
