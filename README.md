...
#### Dead Letter Queue
Each consumer handles failures in two tiers: transient conflicts are retried silently at the application level, while other failures are retried with exponential backoff at the Kafka level. After all retries are exhausted, the message is moved to a dedicated `<topic>-dlq` topic and the consumer continues processing subsequent messages without blocking.
