...
#### Saga Timeout
A scheduled timeout process runs every 10 seconds and finds sagas where the step deadline has passed (30 seconds per step). Timed-out `WAITING_PAYMENT` sagas cancel the order. Timed-out `WAITING_INVENTORY` sagas cancel the order and trigger a `payment-rollback` to reverse the charge. Timed-out `WAITING_ROLLBACK` sagas force-cancel without waiting for rollback confirmation — handling the case where the payment service is unavailable during compensation. This prevents orders from being stuck in a pending state forever if a downstream service is unavailable. The saga query also uses `FOR UPDATE SKIP LOCKED` so multiple pods running the timeout job never process the same expired saga concurrently.
